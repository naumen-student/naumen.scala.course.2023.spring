object Exercises {
  implicit class ScopeFunctions[T](self: T) { //oh hi kotlin scope functions!
  ...
  def divBy3Or7(iFrom: Int, iTo: Int): Seq[Int] = {
    for {i <- iFrom to iTo if i % 3 == 0 || i % 7 == 0} yield i
  }
  ...
  на 3 или на 5.*/
  /*Реализовать юнит-тесты в src/test/scala для данной функции.*/
  def sumOfDivBy3Or5(iFrom: Int, iTo: Int): Long = (iFrom to iTo).filter { it => it % 3 == 0 || it % 5 == 0}.sum
  ...
  def primeFactor(number: Int): Seq[Int] = {
    var n = number
    (2 to number)
      .filter { it => { (2 until it).forall { it % _ != 0 } } }
      .filter { it => { (n % it == 0).also { _ => while (n % it == 0) n /= it } } }
  }
  ...
  */
  ...
  def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
    prettify(sortByWeight(ballsArray))
  }

  def sortByWeight(balls: Map[String, (Int, Double)]): Seq[(String, Double)] = {
    balls.toList.map { it => (it._1, it._2._2 * (4 * (Math.PI * Math.pow(it._2._1, 3)) / 3)) }.sortBy(_._2)
  }

  def prettify(balls: Seq[(String, Double)]): Seq[String] = balls.map(_._1)
}