...
object Exercises {
  ...
  на 3 или на 5.*/
  /*Реализовать юнит-тесты в src/test/scala для данной функции.*/
  def sumOfDivBy3Or5(iFrom: Int, iTo: Int): Long = (for {i <- iFrom to iTo if i % 3 == 0 || i % 5 == 0} yield i).sum
  ...
  def primeFactor(number: Int): Seq[Int] = {
    var n = number
    val res: mutable.SortedSet[Int] = mutable.SortedSet()
    while (n % 2 == 0) {
      res += 2
      n /= 2
    }
    for (i <- 3 until math.sqrt(n).toInt + 1 by 2)
      while (n % i == 0) {
        res += i
        n /= i
      }
  ...
  def sumByFunc(leftVec0: Vector2D, leftVec1: Vector2D, foo: (Vector2D, Vector2D) => Double, rightVec0: Vector2D, rightVec1: Vector2D): Double =
    foo(leftVec0, leftVec1) + foo(rightVec0, rightVec1)
  ...
  */
  ...
  def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] =
    ballsArray.map { case (title, (r, d)) => (title, math.pow(r, 3) * d) }.toSeq.sortBy(_._2).map(_._1)
}