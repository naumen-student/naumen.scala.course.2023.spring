object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        def getRemainder(num: Int, divider: Int, dividers: Seq[Int]): Seq[Int] = {
            if (divider <= num) {
                if (num % divider == 0) {
                    if (!dividers.contains(divider)) {
                        return getRemainder(num / divider, divider, dividers :+ divider)
                    }
                    return getRemainder(num / divider, divider, dividers)
                }
                return getRemainder(num, divider + 1, dividers)
            }
            else {
                return dividers
            }
        }
        return getRemainder(number, 2, Seq[Int]())
    }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        def getWeight(radius: Int, density: Double): Double = {
            java.lang.Math.PI * 4 / 3 * java.lang.Math.pow(radius, 3) * density
        }

        val ballsWeight = ballsArray.map(element => (element._1, getWeight(element._2._1, element._2._2))).toList
        val sortedBalls = ballsWeight.sortBy(element => element._2)
        sortedBalls.map(element => element._1)
    }
