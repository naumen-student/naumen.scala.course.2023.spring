object Exercises {
    ...
    def sumOfDivBy3Or5(iFrom: Int, iTo: Int): Long = {
        var sumOfDiv = 0
        for (i <- iFrom to iTo if i % 3 == 0 || i % 5 == 0) sumOfDiv += i
        sumOfDiv
    }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        ballsArray.toSeq.sortWith((ball1, ball2) => (Math.pow(ball1._2._1, 3) * ball1._2._2) < (Math.pow(ball2._2._1, 3) * ball2._2._2)).map(ball => ball._1)
    }
