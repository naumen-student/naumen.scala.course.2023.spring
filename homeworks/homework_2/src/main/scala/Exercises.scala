object Exercises {
    ...
    def sumByFunc(leftVec0: Vector2D, leftVec1: Vector2D,
                  ...
                  rightVec0: Vector2D, rightVec1: Vector2D): Double = f(leftVec0, leftVec1) + f(rightVec0, rightVec1)
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        val getWeight = (element: (String, (Int, Double))) => {
            val volume = 4 / 3 * Math.PI * Math.pow(element._2._1, 3)
            volume * element._2._2
        }
