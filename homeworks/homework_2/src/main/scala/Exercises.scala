...
object Exercises {
    ...
    {
        val result: mutable.HashSet[Int] = mutable.HashSet()
        var denominator: Int = number
        var multiplier: Int = 2

        while (multiplier * multiplier <= denominator)
        {
    ...
    */
    ...
    {
        ballsArray.keys.toSeq.sortBy(name => 4 / 3 * java.lang.Math.PI * java.lang.Math.pow(ballsArray(name)._1, 3) * ballsArray(name)._2)
    }
}