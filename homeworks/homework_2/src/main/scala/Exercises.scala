...
object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        if (isPrime(number)) return Seq(number)

        for {i <- 2 to number.abs
    ...
    */
    ...
    =
    for {ball <- ballsArray.toSeq.sortBy({case(name, (radius, density))=> weight(radius, density)})} yield ball._1


    def weight(radius: Int, density: Double): Double =
        {
