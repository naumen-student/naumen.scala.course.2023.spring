object Exercises extends App {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        ...
        var primeNumber = 2
        while (residue > 1) {
            while (residue % primeNumber == 0) {
                residue = residue / primeNumber
                if (!res.contains(primeNumber)) res :+= primeNumber
            }
            primeNumber += (if (primeNumber % 2 == 0) 1 else 2)
        }
        res
    }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        ballsArray.map((ball) => (ball._1, (4 / 3f) * java.lang.Math.PI * (ball._2._1 * ball._2._1 * ball._2._1) * ball._2._2))
            .toSeq.sortBy(ball => ball._2).map(ball => ball._1)
    }
}