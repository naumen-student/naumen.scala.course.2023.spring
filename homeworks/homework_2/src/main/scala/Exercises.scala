...
object Exercises {
  ...
  def sumByFunc(leftVec0: Vector2D, leftVec1: Vector2D, sumF: (Vector2D, Vector2D) => Double, rightVec0: Vector2D, rightVec1: Vector2D): Double =
    sumF(leftVec0, leftVec1) + sumF(rightVec0, rightVec1)
  ...
  */
  ...
  def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
    ballsArray.toSeq
      .sortWith((b1, b2) => calculateBallWeight(b1) < calculateBallWeight(b2))
      .map(ball => ball._1)
  }

  def calculateBallWeight(b: (String, (Int, Double))): Double = {
    4 / 3 * Math.PI * Math.pow(b._2._1, 3) * b._2._2
  }
}