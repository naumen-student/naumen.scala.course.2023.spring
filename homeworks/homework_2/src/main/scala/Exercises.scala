...
object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        var ans: ListBuffer[Int] = ListBuffer()
        var n = number
    ...
    def sumByFunc(leftVec0: Vector2D, leftVec1: Vector2D, func: (Vector2D, Vector2D) => Double, rightVec0: Vector2D, rightVec1: Vector2D): Double = {
        val a = func(leftVec0, leftVec1)
        val b = func(rightVec0, rightVec1)
        a + b
    }
    ...
    */
    ...
    private def getBallMass(rAndD : (Int, Double)): Double = {
        rAndD._2*(4/3 * Pi * rAndD._1 * rAndD._1 * rAndD._1)
    }

    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        val sortedBalls = ballsArray.toList.sortWith( (x,y) => getBallMass(x._2) < getBallMass(y._2) )
