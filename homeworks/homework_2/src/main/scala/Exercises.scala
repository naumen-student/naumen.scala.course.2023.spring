...
object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        @tailrec
        def factors(num: Int, acc: List[Int]): List[Int] = {
            if (num == 1) acc.reverse
            else {
                val primeFactor = (2 to num).find(num % _ == 0).get
                factors(num / primeFactor, primeFactor :: acc)
            }
        }

        factors(number, Nil).distinct.filter(i => (2 until i).forall(i % _ != 0))
    }

println(primeFactor(0))
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        ballsArray.toSeq
          .map { case (name, (radius, density)) => (name, 4 / 3 * Pi * radius * radius * radius * density) }
