object Exercises {
    ...
    def isSimple(c: Int): Boolean = {
        var limit = Math.floor(Math.sqrt(c)).toInt
        for (i <- 2 to limit
            if c % i == 0)
                return false
        return true
    ...
    def primeFactor(number: Int): Seq[Int] = {
        var limit = Math.floor(Math.sqrt(number)).toInt
        for {i <- 2 to limit
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = 
        ballsArray.map {case (m, (r, d)) => (m, math.pow(r, 3) * d)}.toSeq.sortBy(_._2).map(_._1)
}