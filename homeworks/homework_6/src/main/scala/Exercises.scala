object Exercises {
  ...
  def wordReverse(text: String): String = {
    ...
    val lastLetter =
      if (word.head.isUpper) word.last.toUpper else word.last.toLower
    val reversedWord =
      lastLetter + word.take(word.length - 1).drop(1).reverse + firstLetter

    reversedWord + separator + wordReverse(
      text.drop(word.length + separator.length)
    )
  }
