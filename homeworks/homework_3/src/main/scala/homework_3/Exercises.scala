...
object Exercises {
    ...
    def prettyBooleanFormatter2(x: Any): String = {
        if (x.isInstanceOf[Boolean]) {
            if (x.asInstanceOf[Boolean]) "true" else "false"
        } else x.toString
    }

    def prettyBooleanFormatter3(x: Any): String = {
        if (x.getClass == Boolean) {
            if (x.asInstanceOf[Boolean]) "true" else "false"
        } else x.toString
    }
    ...
    case class MyIter(x: Int, y: Int) extends Iterable[Int] {
        override def iterator: Iterator[Int] = Iterator(x,y)
    }

    def sum1(x: Int, y: Int): Int = sumIntegers(Seq(x,y))
    def sum2(x: Int, y: Int): Int = sumIntegers(MyIter(x,y))
    def sum3(x: Int, y: Int): Int = sumIntegers(MyIter(x,y))
