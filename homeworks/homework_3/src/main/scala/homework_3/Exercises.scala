...
object Exercises {
    ...
    def prettyBooleanFormatter3(x: Any): String = if (x.getClass.getTypeName == "Boolean") if (x.asInstanceOf[Boolean]) "правда" else "ложь" else x.toString
     ...
     */
    def max1(xs: Seq[Int]): Int = if (xs.nonEmpty) xs.max else throw new IllegalArgumentException()
