...
object Exercises {
    ...
    def prettyBooleanFormatter2(x: Any): String = {
        if (x.isInstanceOf[java.lang.Boolean])
            if (x.asInstanceOf[Boolean]) "правда" else "ложь"
        else x.toString
    }
    ...
    def max1(xs: Seq[Int]): Int = {
        if (xs.isEmpty) throw new IllegalArgumentException("Cannot find max value of empty sequence")
        else xs.max
    }
    ...
    case class TwoValuesIterable(x: Int, y: Int) extends Iterable[Int] {
    ...
    def sum2(x: Int, y: Int): Int = sumIntegers(TwoValuesIterable(x, y))
