...
object Exercises {
    ...
    def prettyBooleanFormatter2(x: Any): String = x.isInstanceOf[Boolean] match {
    ...
    def sum2(x: Int, y: Int): Int = sumIntegers(Array(x, y).toSeq)

    def sum3(x: Int, y: Int): Int = sumIntegers(Vector(x, y))

    case class Vector(x: Int, y: Int) extends Iterable[Int] {

      override def iterator: Iterator[Int] = Seq(x, y).iterator
    }
}
