...
object Exercises {
    ...
    def prettyBooleanFormatter1(x: Any): String =
        x match {
            case bool: Boolean => return if (bool) "правда" else "ложь"
            case _ => x.toString
        }
    ...
    def max1(xs: Seq[Int]): Int =
        {
            if (xs.isEmpty)
                throw new IllegalArgumentException("empty.max")
            else
                xs.max
        }
     ...
     */
    case class IntIterable(x:Int,y:Int) extends Iterable[Int]
    {
    ...
    def sum3(x: Int, y: Int): Int = sumIntegers(IntIterable(x,y))
