...
object Test extends TestSuite {
    val tests = Tests {
        'test_createTable - {
            ...
            assert(table.getCell(0, -1).map(_.toString).isEmpt)
            assert(table.getCell(-1, 0).map(_.toString).isEmpt)
            assert(table.getCell(9, 8).map(_.toString).isEmpt)
            assert(table.getCell(8, 9).map(_.toString).isEmpt)
        }
        'test_numberCell - {
