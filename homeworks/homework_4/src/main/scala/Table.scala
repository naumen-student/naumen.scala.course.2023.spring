...
class Table(width: Int, height: Int) {
    ...
    def setCell(ix: Int, iy: Int, cell: Cell[_]): Unit = 
        if (!containsCell(ix, iy))
            throw new IndexOutOfBoundsException(s"Cannot insert cell, indexes $ix and $iy are not in the table")
        else matrix(getIndex(ix, iy)) = cell

    def containsCell(ix: Int, iy: Int): Boolean = matrix.indices.contains(getIndex(ix, iy))
}