...
object ListOps {
  ...
  def foldOption[T](f: (T, T) => T): DataList[T] => Option[T] = {
    ...
    case NonEmptyList(head, NonEmptyList(tailHead, tail)) =>
      foldOption(f)(NonEmptyList(f(head, tailHead), tail))
  }
  ...
  @tailrec
  private def reverseImpl[T](buffer: DataList[T])(l: DataList[T]): DataList[T] =
    l match {
      case EmptyList => buffer
      case NonEmptyList(head, tail) =>
        reverseImpl(NonEmptyList(head, buffer))(tail)
    }
  ...
  private def filterImpl[T](
      f: T => Boolean
  )(buffer: DataList[T])(l: DataList[T]): DataList[T] = l match {
