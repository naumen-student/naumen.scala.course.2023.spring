...
object ListOps {
  ...
  def foldOption[T](f: (T, T) => T): DataList[T] => Option[T] = {
    ...
    case list => list match {
      case NonEmptyList(current, EmptyList) => Some(current)
      case NonEmptyList(current, NonEmptyList(prev, tail)) => foldOption(f)(NonEmptyList(f(current, prev), tail))
    }
  }
   ...
   */
  def collect[A, B](f: PartialFunction[A, B]): DataList[A] => DataList[B] = list => map(f)(filter(f.isDefinedAt)(list))
}