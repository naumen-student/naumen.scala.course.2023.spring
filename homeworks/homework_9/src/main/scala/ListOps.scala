...
object ListOps {
  ...
  @tailrec
  private def reverse[T](buffer: DataList[T] = EmptyList)(l: DataList[T]): DataList[T] = l match {
   ...
   */
  def collect[A, B](f: PartialFunction[A, B]): DataList[A] => DataList[B] = list =>
    map(f)(filter(f.isDefinedAt)(list))
}